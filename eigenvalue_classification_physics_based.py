...
Physics-based eigenvalue → eigenmode association.

(Full script; minimal edit: skip kinematic integrator eigenpairs (zeros) when classifying).
"""
...
# ---------------------------
def match_evals_to_emodes(matfile, eigfile):
...
# If run as script, small demonstration (example)
if __name__ == "__main__":
    ...
    # Example plotting of Re/Im for row_index=1 across categories (keeps your original plotting approach)
    modes = ['roll', 'short-period', 'dutch-roll', 'phugoid', 'spiral']
    fig = plt.figure(figsize=(15,9), constrained_layout=True)
    gs = gridspec.GridSpec(2, 3, width_ratios=[1,1,1], hspace=0.2, height_ratios=[1,1], wspace=0.3)
    axes = [fig.add_subplot(gs[_i,_j]) for _i in range(2) for _j in range(3)]
    for row_idx, ax in enumerate(axes[:6]):
        rid = row_idx + 1
        for mode in modes:
            # plot for the three categories combined
            for cat in ['fuselage','wing','nacelle']:
                df_cat = res[cat][mode][rid]
                ax.scatter(df_cat['Re'], df_cat['Im'], marker='.', s=8, alpha=0.7)
        ax.set_xlim(-1.5, 0.5); ax.set_title(f"row_index={rid}")
    plt.show()