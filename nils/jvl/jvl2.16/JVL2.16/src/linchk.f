...
c        deljet(n) = 0.    !###
      ...
      do iter = 1, niter+2
        call setup
ccc     call sgetrf(nvor,nvmax,aicn,nvmax,indx,info)
        call ludcmp(nvmax,nvor,aicn,indx,work)
        do i = 1, nvor
          dgam(i) = -resn(i)
        enddo
ccc     call sgetrs('N',nvor,1,aicn,nvmax,indx,dgam,nvmax,info)
...
ccc   call sgetrf(nvor,nvmax,aicn,nvmax,indx,info)
      ...
      do n = 1, numax
ccc     call sgetrs('N',nvor,1,aicn,nvmax,indx,gam_u(1,n),nvmax,info)
        call baksub(nvmax,nvor,aicn,indx,gam_u(1,n))
      enddo
      do n = 1, ncontrol
ccc     call sgetrs('N',nvor,1,aicn,nvmax,indx,gam_d(1,n),nvmax,info)
        call baksub(nvmax,nvor,aicn,indx,gam_d(1,n))
      enddo
      do n = 1, nvarjet
ccc     call sgetrs('N',nvor,1,aicn,nvmax,indx,gam_j(1,n),nvmax,info)
        call baksub(nvmax,nvor,aicn,indx,gam_j(1,n))
      enddo
      do n = 1, ndesign
ccc     call sgetrs('N',nvor,1,aicn,nvmax,indx,gam_g(1,n),nvmax,info)
