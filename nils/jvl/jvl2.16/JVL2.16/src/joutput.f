...
c...print out the results -> forces by surface and strip
 ...
 211    format (/' Surface forces referred to Sref,Cref,Bref ',
     &        'about Xmom,Ymom,Zmom'/
     &         ' ',a)
 212    format (
     &       /2x,'Surface #',i2,5x,a/
 ...
 213    format (
  ...
  216   format (/' Strip Forces referred to strip area, chord'/
     ...
     &          9x,'Chord',7x,'Area',8x,'Dihedral',
     &          3x,'c cl',8x,'ai_ff',7x,'cl',11x,'clj',9x,'cd',
     &          10x,'cdv',9x,'cm_c/4',5x,'c.p.x/c',6x,'Dcj')
...
c...print out the results -> forces by surface and strip
  ...
  212   format (/2x,'Surface #',i2,5x,a/
     ...
     &         5x,'First strip =',i3/
     &        5x,'Surface area =',g12.5,5x,'  ave. chord =',g12.5)
  213   format (/' Body Axes Forces referred to Sref,Cref,Bref ',
     &        'about Xref,Yref,Zref'/' ',A)
  214   format ( 5X,'CFXsurf  =',G12.5,5X,'CMXsurf  =',G12.5,
  ...
  216   format (/' Body axes strip Forces referred to strip area, chord'
     ...
     &          3x,'CFXstrp',5x,'CFYstrp',5x,'CFZstrp',
     &          5x,'CMXstrp',5x,'CMYstrp',5x,'CMZstrp')
...
c      ccl_strp = ( ulift(1)*(fsi(1,j)+fsj(1,j))
c     &           + ulift(2)*(fsi(2,j)+fsj(2,j))
c     &           + ulift(3)*(fsi(3,j)+fsj(3,j)) ) / (0.5*rho*wstrip(j))
c---- calculate force normal to plane of surface
      ccl_strp = ( ensy(j)*fsi(2,j)
     &           + ensz(j)*fsi(3,j) ) / (0.5*rho*wstrip(j))
