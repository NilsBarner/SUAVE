...
C
        WRITE (LUN,211) N,STITLE(N),NV,NS,J1,SSURF(N),CAVESURF(N)
        WRITE (LUN,212) SATYPE
        WRITE (LUN,213) DIR*CXSURF(N),DIR*CRSURF(N),
     &                  CYSURF(N),        CMSURF(N),
     &                  DIR*CZSURF(N),DIR*CNSURF(N)
C
        WRITE (LUN,215) 
        WRITE (LUN,216) 
        DO JJ = 1, NS
     ...
     &           CHORD(J),ASTRP,DIHED,
     &           DIR*CF_STRP(1,J),CF_STRP(2,J),DIR*CF_STRP(3,J),
     &           DIR*CM_STRP(1,J),CM_STRP(2,J),DIR*CM_STRP(3,J)
C
         END DO
      END DO
      WRITE (LUN,200)
...
C
 215  FORMAT (/' Body Axes Strip Forces referred to Strip Area, Chord,',
  ...
  216 FORMAT ( 2X,'  j ',5X,'Xc/4',7X,'Yc/4',7X,'Zc/4',
     &         7X,'Chord',6X,'Area',5X,'Dihedral',4X,
     &         2X,'CFXstrp',6X,'CFYstrp',6X,'CFZstrp',
     &         6X,'CMXstrp',6X,'CMYstrp',6X,'CMZstrp')
  217 FORMAT (2X,I4,3(1X,F10.5),3(1X,F10.5),3X,6(1X,G12.5))
C
      RETURN
      END ! OUTSTRPB
...
C
 ...
 8125 FORMAT(' x force     |',20(A,'  CDd',I2.2,' =',F11.6))
C
...
C
      WRITE(LU,8125) (' ',K,CDTOT_D(K), K=1, NCONTROL)
 8125 FORMAT(' x  force CD |'  ,20(A,'  CDd',I2.2,' =',F11.6))
C
      WRITE(LU,8140) (' ',K,DIR*CRTOT_D(K), K=1, NCONTROL)
