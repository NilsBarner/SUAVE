...
"""
THE INDICES IN THIS FILE HAVE BEEN UPDATED SUBSTANTIALLY
BY NILS TO COMPLY WITH AVL 3.52 (2025)!
"""
...
def read_results(avl_object):
    ...
    for case in avl_object.current_status.cases:
        ...
        with open(case.aero_result_filename_1,'r') as stab_der_vile:
            ...
            case_res.aerodynamics.roll_moment_coefficient                   = float(lines[19][31:42].strip())
            case_res.aerodynamics.pitch_moment_coefficient                  = float(lines[20][31:42].strip())
            case_res.aerodynamics.yaw_moment_coefficient                    = float(lines[21][31:42].strip())
            ...
            # <<< NILS: from here on the latest version of AVL has a slightly different output file format >>>
            ...
            # NILS: note that CDa and CDb are not used by SUAVE (probably not available in the version of AVL used at the time)
            ...
            case_res.stability.CY_p                                         = float(lines[46+num_ctrl][23:34].strip())
            ...
            if num_ctrl != 0: 
                for ctrl_idx in range(num_ctrl):
                    ...
                    ctrl_surf.Cl                  = float(lines[56+num_ctrl][(21*ctrl_idx + 24):(21*ctrl_idx + 35)].strip())
                    ctrl_surf.Cm                  = float(lines[57+num_ctrl][(21*ctrl_idx + 24):(21*ctrl_idx + 35)].strip())
                    ctrl_surf.Cn                  = float(lines[58+num_ctrl][(21*ctrl_idx + 24):(21*ctrl_idx + 35)].strip())
                    ctrl_surf.CDff                = float(lines[59+num_ctrl][(21*ctrl_idx + 24):(21*ctrl_idx + 35)].strip())
                    ctrl_surf.e                   = float(lines[60+num_ctrl][(21*ctrl_idx + 24):(21*ctrl_idx + 35)].strip())
