...
def write_run_cases(avl_object,trim_aircraft):
    ...
    Properties Used:
        N/A
    """       
    # NILS: creates files like `batch_01.run`
...
def make_controls_case_text(cs_names,avl_aircraft):
    ...
    """ 
    # NILS: only called when control surfaces are defined
    control_surface_text = []
    
    for wing in avl_aircraft.wings: 
        for ctrl_surf in wing.control_surfaces:
            if (type(ctrl_surf) ==  Slat):  # NILS: calls slats "leading edge flaps" (see page 2 in AVL_User_Primer.pdf)
                pass
            else:
